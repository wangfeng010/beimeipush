...
def data_preprocess(df: pd.DataFrame, config: AppConfig):
    """
    ...
    # 遍历配置中的每个处理管道
    piplines = (
        config.features.process.pipelines + config.features.label_process.pipelines
    )
    ...
    # 获取变长特征的最大列数
    max_col_num = config.serve.data_handling.data_format.varlen_max_col_num
